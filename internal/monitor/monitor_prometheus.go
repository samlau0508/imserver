...
type Prometheus struct {
	...
	connNumFifo         *okutil.FIFO
	sample              int // 采样数量
	upstreamPackageFifo *okutil.FIFO
	upstreamPackageQPS  atomic.Int64 // 上行数据包QPS

	upstreamTrafficFifo *okutil.FIFO // 上行流量
	upstreamTrafficQPS  atomic.Int64 // 上行流量QPS

	downstreamPackageFifo *okutil.FIFO
	downstreamPackageQPS  atomic.Int64 // 下行数据包QPS

	downstreamTrafficFifo *okutil.FIFO // 下行流量
...
func NewPrometheus() IMonitor {
	...
	return &Prometheus{
		...
		upstreamTrafficFifo:           okutil.NewFIFO(sample),
		...
		downstreamTrafficFifo:           okutil.NewFIFO(sample),
		...
		connNumFifo:               okutil.NewFIFO(sample),
		upstreamPackageFifo:       okutil.NewFIFO(sample),
		downstreamPackageFifo:     okutil.NewFIFO(sample),
		namespace:                 namespace,
		subsystem:                 subsystem,
		sample:                    sample,
		// ---------------- db -----------------
