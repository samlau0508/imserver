...
func newConnInfo(c oknet.Conn) *ConnInfo {
	var (
		now  = time.Now()
		host string
		port int
...
func device(conn oknet.Conn) string {
	d := "未知"
	level := "主"
	switch okproto.DeviceFlag(conn.DeviceFlag()) {
	case okproto.APP:
		d = "App"
	case okproto.PC:
		d = "PC"
	...
	if okproto.DeviceLevel(conn.DeviceLevel()) == okproto.DeviceLevelSlave {
		level = "从"
	}
...
// byID
type byID struct{ Conns []oknet.Conn }
...
// byIDDesc
type byIDDesc struct{ Conns []oknet.Conn }
...
// inMsg
type byInMsg struct{ Conns []oknet.Conn }
...
type byInMsgDesc struct{ Conns []oknet.Conn }
...
type byOutMsg struct{ Conns []oknet.Conn }
...
type byOutMsgDesc struct{ Conns []oknet.Conn }
...
type byInBytes struct{ Conns []oknet.Conn }
...
type byInBytesDesc struct{ Conns []oknet.Conn }
...
type byOutBytes struct{ Conns []oknet.Conn }
...
type byOutBytesDesc struct{ Conns []oknet.Conn }
...
type byPendingBytes struct{ Conns []oknet.Conn }
...
type byPendingBytesDesc struct{ Conns []oknet.Conn }
...
type byUptime struct{ Conns []oknet.Conn }
...
type byUptimeDesc struct{ Conns []oknet.Conn }
