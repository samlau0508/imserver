...
func NewConversationAPI(s *Server) *ConversationAPI {
	return &ConversationAPI{
		s:   s,
		Log: oklog.NewOKLog("ConversationAPI"),
	}
}
...
func (s *ConversationAPI) conversationsList(c *okhttp.Context) {
	uid := c.Query("uid")
	...
	if len(conversations) > 0 {
		for _, conversation := range conversations {
			...
			if conversation.ChannelType == okproto.ChannelTypePerson {
...
// 清楚会话未读数量
func (s *ConversationAPI) clearConversationUnread(c *okhttp.Context) {
...
func (s *ConversationAPI) setConversationUnread(c *okhttp.Context) {
	var req struct {
		UID         string `json:"uid"`
...
func (s *ConversationAPI) deleteConversation(c *okhttp.Context) {
...
func (s *ConversationAPI) syncUserConversation(c *okhttp.Context) {
	var req struct {
		UID         string             `json:"uid"`
		...
		Larges      []*okproto.Channel `json:"larges"`        // 超大频道集合
	}
	if err := c.BindJSON(&req); err != nil {
	...
	var newConversations = make([]*okstore.Conversation, 0, len(conversations)+20)
	...
	if len(req.Larges) > 0 && req.MsgCount > 0 {
		for _, largeChannel := range req.Larges {
			...
			if existConversation != nil {
				...
				if lastMessage != nil {
					newConversations = append(newConversations, &okstore.Conversation{
						UID:             req.UID,
...
func (s *ConversationAPI) syncRecentMessages(c *okhttp.Context) {
	var req struct {
		UID      string                     `json:"uid"`
...
func (s *ConversationAPI) getRecentMessages(uid string, msgCount int, channels []*channelRecentMessageReq) ([]*channelRecentMessage, error) {
	...
	if len(channels) > 0 {
		var (
			recentMessages []okstore.Message
			err            error
		)
		for _, channel := range channels {
			fakeChannelID := channel.ChannelID
			if channel.ChannelType == okproto.ChannelTypePerson {
