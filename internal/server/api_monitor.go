...
func (m *MonitorAPI) realtime(c *okhttp.Context) {
	last := c.Query("last")
...
func (m *MonitorAPI) writeDataToMonitor(typ string, dataCallback func(subscribeInfo *okstore.SubscribeInfo) interface{}) {
	monitorChannel, err := m.s.channelManager.GetChannel(fmt.Sprintf("%s_%s", m.monitorChannelPrefix, typ), okproto.ChannelTypeData)
	if err != nil {
	...
	for _, subscriber := range subscribers {
		...
		for _, conn := range conns {
			...
			rDataBytes = []byte(okutil.ToJSON(data))
...
func (m *MonitorAPI) varzPublish() {
	m.writeDataToMonitor("varz", func(subscriberInfo *okstore.SubscribeInfo) interface{} {
		show := ""
...
func (m *MonitorAPI) realtimePublish() {
	m.writeDataToMonitor("realtime", func(subscriberInfo *okstore.SubscribeInfo) interface{} {
...
func (m *MonitorAPI) connzPublish() {
	...
	m.writeDataToMonitor("connz", func(subscriberInfo *okstore.SubscribeInfo) interface{} {
		if subscriberInfo != nil && subscriberInfo.Param != nil {
...
func (m *MonitorAPI) channels(c *okhttp.Context) {
	...
	slotNum := okutil.GetSlotNum(m.s.opts.SlotNum, topic)
...
func (m *MonitorAPI) messages(c *okhttp.Context) {
	channelID := c.Query("channel_id")
	channelTypeI, _ := strconv.Atoi(c.Query("channel_type"))
	channelType := uint8(channelTypeI)
...
func (m *MonitorAPI) conversations(c *okhttp.Context) {
	uid := c.Query("uid")
