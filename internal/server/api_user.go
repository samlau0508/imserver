...
// 强制设备退出
func (u *UserAPI) deviceQuit(c *okhttp.Context) {
	var req struct {
	...
	if req.DeviceFlag == -1 {
		u.quitUserDevice(req.UID, okproto.APP)
		u.quitUserDevice(req.UID, okproto.WEB)
		u.quitUserDevice(req.UID, okproto.PC)
	} else {
		u.quitUserDevice(req.UID, okproto.DeviceFlag(req.DeviceFlag))
	}
...
func (u *UserAPI) quitUserDevice(uid string, deviceFlag okproto.DeviceFlag) error {
	err := u.s.store.UpdateUserToken(uid, deviceFlag.ToUint8(), uint8(okproto.DeviceLevelMaster), "") // 这里的deviceLevel可以随便给 不影响逻辑 这里随便给的master
	if err != nil {
	...
	if len(oldConns) > 0 {
		for _, oldConn := range oldConns {
			u.s.dispatch.dataOut(oldConn, &okproto.DisconnectPacket{
...
func (u *UserAPI) getOnlineStatus(c *okhttp.Context) {
	var uids []string
...
// 更新用户的token
func (u *UserAPI) updateToken(c *okhttp.Context) {
	...
	channelInfo, err := u.s.store.GetChannel(req.UID, okproto.ChannelTypePerson)
	if err != nil {
	...
	if ban {
		c.ResponseStatus(int(okproto.ReasonBan))
		return
	}
	...
	if req.DeviceLevel == okproto.DeviceLevelMaster {
		// 如果存在旧连接，则发起踢出请求
...
// 添加系统uid
func (u *UserAPI) systemUIDsAdd(c *okhttp.Context) {
	var req struct {
...
// 移除系统uid
func (u *UserAPI) systemUIDsRemove(c *okhttp.Context) {
	var req struct {
		UIDs []string `json:"uids"`
	}
	if err := c.BindJSON(&req); err != nil {
