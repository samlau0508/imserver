...
import (
	...
	proto "github.com/samlau0508/imserver/pkg/proto"
	"go.uber.org/zap"
)
...
func NewChannel(channelInfo *okstore.ChannelInfo, s *Server) *Channel {
	...
	return &Channel{
		...
		s:                s,
		Log:              oklog.NewOKLog(fmt.Sprintf("channel[%s-%d]", channelInfo.ChannelID, channelInfo.ChannelType)),
	}
}
...
func (c *Channel) initChannelInfo() error {
	...
	if c.ChannelType == proto.ChannelTypePerson {
		return nil
	}
...
func (c *Channel) initSubscribers() error {
	...
	if c.s.opts.HasDatasource() && channelType != proto.ChannelTypePerson {
	...
	if channelType == proto.ChannelTypeCustomerService {
...
func (c *Channel) parentChannel() (*Channel, error) {
	...
	if c.ChannelType == proto.ChannelTypeCommunityTopic {
		parentChannelID := GetCommunityTopicParentChannelID(c.ChannelID)
		if parentChannelID != "" {
			var err error
			parentChannel, err = c.s.channelManager.GetChannel(parentChannelID, proto.ChannelTypeCommunity)
			if err != nil {
				return nil, err
			}
...
func (c *Channel) Allow(uid string) (bool, proto.ReasonCode) {
	...
	if c.ChannelType == proto.ChannelTypeInfo { // 资讯频道都可以发消息
		return true, proto.ReasonSuccess
	}
	...
	if c.Ban { // 频道被封
		return false, proto.ReasonBan
	}

	if c.ChannelType == proto.ChannelTypePerson && c.s.opts.IsFakeChannel(c.ChannelID) {
		if c.IsDenylist(uid) {
			return false, proto.ReasonInBlacklist
		}
		return true, proto.ReasonSuccess
	}
	if c.ChannelType != proto.ChannelTypePerson || c.s.opts.WhitelistOffOfPerson == 0 {
		whitelistLength := 0
		...
		if whitelistLength > 0 { // 如果白名单有内容，则只判断白名单
			...
			if ok {
				return ok, proto.ReasonSuccess
			}
			return ok, proto.ReasonNotInWhitelist
		}
		if c.ChannelType == proto.ChannelTypePerson { // 个人频道强制验证白名单，除非WhitelistOffOfPerson==1
			if whitelistLength == 0 {
				return false, proto.ReasonNotInWhitelist
			}
		}
	}
...
func (c *Channel) RealSubscribers(customSubscribers []string) ([]string, error) {
	...
	if c.ChannelType == proto.ChannelTypeCommunityTopic { // 社区话题频道
	...
	} else if c.ChannelType == proto.ChannelTypeInfo {
	...
	return okutil.RemoveRepeatedElement(subscribers), nil
}
...
func (c *Channel) Put(messages []*Message, customSubscribers []string, fromUID string, fromDeviceFlag proto.DeviceFlag, fromDeviceID string) error {
	...
	// ########## update conversation ##########
	if !c.Large && c.ChannelType != proto.ChannelTypeInfo { // 如果是大群 则不维护最近会话 几万人的大群，更新最近会话也太耗性能
		var lastMsg *Message
...
func (c *Channel) storeMessageToUserQueueIfNeed(messages []*Message, subscribers []string) (map[string]uint32, error) {
	...
	for _, subscriber := range subscribers {
		...
		for _, m := range messages {
			...
			if m.ChannelType == proto.ChannelTypePerson && m.ChannelID == subscriber {
