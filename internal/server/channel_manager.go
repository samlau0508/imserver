...
func NewChannelManager(s *Server) *ChannelManager {
	...
	return &ChannelManager{
		...
		s:                s,
		Log:              oklog.NewOKLog("ChannelManager"),
	}
}
...
func (cm *ChannelManager) GetChannel(channelID string, channelType uint8) (*Channel, error) {
	...
	if channelType == okproto.ChannelTypeData {
...
func (cm *ChannelManager) getChannelFromCacheOrStore(channelID string, channelType uint8) (*Channel, error) {
	...
	if channelInfo == nil {
		if cm.s.opts.Channel.CreateIfNoExist || channelType == okproto.ChannelTypeCommunityTopic || channelType == okproto.ChannelTypeInfo {
			channelInfo = okstore.NewChannelInfo(channelID, channelType)
		} else {
			return nil, nil
		}
	}
...
func (cm *ChannelManager) CreateOrUpdatePersonChannel(uid string) error {
	exist, err := cm.s.store.ExistChannel(uid, okproto.ChannelTypePerson)
	if err != nil {
		return errors.Wrap(err, "查询是否存在频道信息失败！")
	}
	if !exist {
		err = cm.s.store.AddOrUpdateChannel(&okstore.ChannelInfo{
			ChannelID:   uid,
	...
	subscribers, err := cm.s.store.GetSubscribers(uid, okproto.ChannelTypePerson)
	if err != nil {
		return errors.Wrap(err, "获取频道订阅者失败！")
	}
	if len(subscribers) == 0 || !okutil.ArrayContains(subscribers, uid) {
		err = cm.s.store.AddSubscribers(uid, okproto.ChannelTypePerson, []string{uid})
		if err != nil {
...
func (cm *ChannelManager) CreateTmpChannel(channelID string, channelType uint8, subscribers []string) error {
	channel := NewChannel(okstore.NewChannelInfo(channelID, channelType), cm.s)
