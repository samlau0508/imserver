...
func GetFakeChannelIDWith(fromUID, toUID string) string {
	...
	fromUIDHash := okutil.HashCrc32(fromUID)
	toUIDHash := okutil.HashCrc32(toUID)
	if fromUIDHash > toUIDHash {
		return fmt.Sprintf("%s@%s", fromUID, toUID)
	}
	if fromUIDHash == toUIDHash {
		oklog.Warn("生成的fromUID的Hash和toUID的Hash是相同的！！", zap.Uint32("fromUIDHash", fromUIDHash), zap.Uint32("toUIDHash", toUIDHash), zap.String("fromUID", fromUID), zap.String("toUID", toUID))
	}
...
// 加密消息
func encryptMessagePayload(payload []byte, conn oknet.Conn) ([]byte, error) {
	var (
		aesKey = conn.Value(aesKeyKey).(string)
		aesIV  = conn.Value(aesIVKey).(string)
	)
	// 加密payload
	payloadEnc, err := okutil.AesEncryptPkcs7Base64(payload, []byte(aesKey), []byte(aesIV))
	if err != nil {
		return nil, err
	}
...
func makeMsgKey(signStr string, conn oknet.Conn) (string, error) {
	var (
		aesKey = conn.Value(aesKeyKey).(string)
		aesIV  = conn.Value(aesIVKey).(string)
	)
	// 生成MsgKey
	msgKeyBytes, err := okutil.AesEncryptPkcs7Base64([]byte(signStr), []byte(aesKey), []byte(aesIV))
	if err != nil {
		oklog.Error("生成MsgKey失败！", zap.Error(err))
		return "", err
	}
	return okutil.MD5(string(msgKeyBytes)), nil
}
