...
type connContext struct {
	...
	frameCaches    []okproto.Frame
	s              *Server
	inflightCount  int // frame inflight count
	oklog.Log

	subscriberInfos    map[string]*okstore.SubscribeInfo // 订阅的频道数据, key: channel, value: SubscriberInfo
...
func newConnContext(s *Server) *connContext {
	c := &connContext{
		s:               s,
		frameCacheLock:  sync.RWMutex{},
		Log:             oklog.NewOKLog("connContext"),
		subscriberInfos: make(map[string]*okstore.SubscribeInfo),
	}
	return c
}

func (c *connContext) putFrame(frame okproto.Frame) {
...
func (c *connContext) subscribeChannel(channelID string, channelType uint8, param map[string]interface{}) {
	...
	if c.conn != nil {
		c.subscriberInfos[key] = &okstore.SubscribeInfo{
			UID:   c.conn.UID(),
			Param: param,
		}
	}
...
func (c *connContext) getSubscribeInfo(channelID string, channelType uint8) *okstore.SubscribeInfo {
	c.subscriberInfoLock.RLock()
	defer c.subscriberInfoLock.RUnlock()
	key := fmt.Sprintf("%s-%d", channelID, channelType)
...
func (c *connContext) init() {
	c.frameCaches = make([]okproto.Frame, 0, 250)
	c.Log = oklog.NewOKLog("connContext")
}
