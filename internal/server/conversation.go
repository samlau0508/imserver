...
import (
	...
	"github.com/samlau0508/imserver/pkg/keylock"
...
type ConversationManager struct {
	...
	oklog.Log
	queue                          *Queue
	userConversationMapBuckets     []map[string]*lru.Cache[string, *okstore.Conversation]
...
func NewConversationManager(s *Server) *ConversationManager {
	cm := &ConversationManager{
		s:                       s,
		bucketNum:               10,
		Log:                     oklog.NewOKLog("ConversationManager"),
	...
	cm.userConversationMapBuckets = make([]map[string]*lru.Cache[string, *okstore.Conversation], cm.bucketNum)
...
func (cm *ConversationManager) getUserAllConversationMapFromStore(uid string) ([]*okstore.Conversation, error) {
...
func (cm *ConversationManager) newLRUCache() *lru.Cache[string, *okstore.Conversation] {
	c, _ := lru.New[string, *okstore.Conversation](cm.s.opts.Conversation.UserMaxCount)
	return c
}
...
func (cm *ConversationManager) flushUserConversations(uid string) {
	...
	conversationCache := cm.getUserConversationCache(uid)
	conversations := make([]*okstore.Conversation, 0, len(conversationCache.Keys()))
	for _, key := range conversationCache.Keys() {
...
func (cm *ConversationManager) getUserConversationCache(uid string) *lru.Cache[string, *okstore.Conversation] {
	pos := int(okutil.HashCrc32(uid) % uint32(cm.bucketNum))
	...
	if userConversationMap == nil {
		userConversationMap = make(map[string]*lru.Cache[string, *okstore.Conversation])
...
func (cm *ConversationManager) getUserCacheConversations(uid string) []*okstore.Conversation {
	cache := cm.getUserConversationCache(uid)

	conversations := make([]*okstore.Conversation, 0, len(cache.Keys()))
...
func (cm *ConversationManager) calConversation(message *Message, subscriber string) {
	...
	if message.ChannelType == okproto.ChannelTypePerson && message.ChannelID == subscriber { // If it is a personal channel and the channel ID is equal to the subscriber, you need to swap fromUID and channelID
	...
	if conversation == nil {
		...
		conversation = &okstore.Conversation{
			UID:             subscriber,
			ChannelID:       channelID,
...
func (cm *ConversationManager) GetConversations(uid string, version int64, larges []*okproto.Channel) []*okstore.Conversation {
	...
	newConversations := make([]*okstore.Conversation, 0)
	...
	for _, updateConversation := range updateConversations {
		existIndex := 0
		var existConversation *okstore.Conversation
...
func (cm *ConversationManager) channelInLarges(channelID string, channelType uint8, larges []*okproto.Channel) bool {
	if len(larges) == 0 {
		return false
	}
...
type conversationSlice []*okstore.Conversation
