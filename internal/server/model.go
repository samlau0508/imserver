...
type Message struct {
	*okproto.RecvPacket
	...
	fromDeviceFlag okproto.DeviceFlag // 发送者设备标示
...
func (m *Message) Encode() []byte {
	...
	return okstore.EncodeMessage(m.MessageSeq, data)
}

func (m *Message) Decode(msg []byte) error {
	messageSeq, data, err := okstore.DecodeMessage(msg)
	if err != nil {
		return err
	}
...
func (m *Message) StreamStart() bool {
	...
	return m.StreamFlag == okproto.StreamFlagStart
}
...
func (m *Message) StreamIng() bool {
	...
	return m.StreamFlag == okproto.StreamFlagIng
}
...
func MarshalMessage(version uint8, m *Message) []byte {
	enc := okproto.NewEncoder()
	defer enc.End()
	_ = enc.WriteByte(okproto.ToFixHeaderUint8(m.RecvPacket))
	...
	if m.Setting.IsSet(okproto.SettingStream) {
...
func UnmarshalMessage(data []byte, m *Message) error {
	...
	recvPacket := &okproto.RecvPacket{}
	framer := okproto.FramerFromUint8(header)
	...
	m.Setting = okproto.Setting(setting)
	...
	if m.Setting.IsSet(okproto.SettingStream) {
		...
		m.StreamFlag = okproto.StreamFlag(streamFlag)
	}
	// Timestamp
...
func (m *MessageResp) from(messageD *Message, store okstore.Store) {
	m.Header.NoPersist = okutil.BoolToInt(messageD.NoPersist)
	m.Header.RedDot = okutil.BoolToInt(messageD.RedDot)
	m.Header.SyncOnce = okutil.BoolToInt(messageD.SyncOnce)
	...
	if messageD.ChannelType == okproto.ChannelTypePerson {
	...
	if strings.TrimSpace(messageD.StreamNo) != "" && store != nil {
		...
		if err != nil {
			oklog.Error("获取streamItems失败！", zap.Error(err))
		}
...
func newStreamItemResp(m *okstore.StreamItem) *StreamItemResp {
...
func newSyncUserConversationResp(conversation *okstore.Conversation) *syncUserConversationResp {
