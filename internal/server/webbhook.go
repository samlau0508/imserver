...
import (
	...
	"github.com/samlau0508/imserver/pkg/exhook"
	"github.com/samlau0508/imserver/pkg/grpcpool"
...
func (w *Webhook) notifyOfflineMsg(msg *Message, large bool, subscribers []string) {
	...
	if w.s.opts.Channel.SubscriberCompressOfCount > 0 && len(subscribers) > w.s.opts.Channel.SubscriberCompressOfCount {
		...
		defer gWriter.Close()
		_, err := gWriter.Write([]byte(okutil.ToJSON(subscribers)))
		if err != nil {
		...
		Data: MessageOfflineNotify{
			MessageResp: MessageResp{
				Header: MessageHeader{
					RedDot:    okutil.BoolToInt(msg.RedDot),
					SyncOnce:  okutil.BoolToInt(msg.SyncOnce),
					NoPersist: okutil.BoolToInt(msg.NoPersist),
				},
...
func (w *Webhook) sendWebhookForGRPC(event string, data []byte) error {
	...
	// 	return err
	// }
	cli := exhook.NewWebhookServiceClient(clientConn)

	sendCtx, sendCancel := context.WithTimeout(context.Background(), time.Second*10)
	defer sendCancel()
	resp, err := cli.SendWebhook(sendCtx, &exhook.EventReq{
		Event: event,
		Data:  data,
	})
	...
	if resp.Status != exhook.EventStatus_Success {
