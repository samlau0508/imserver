...
// OnRecv 收到消息事件
type OnRecv func(recv *okproto.RecvPacket) error
...
func New(addr string, opt ...Option) *Client {
	...
	c := &Client{
		...
		opts:  opts,
		proto: okproto.New(),
		Log:   oklog.NewOKLog(fmt.Sprintf("IMClient[%s]", opts.UID)),
		writer: &limWriter{
...
func (c *Client) sendPing(ch chan struct{}) error {
	...
	data, err := c.proto.EncodeFrame(&okproto.PingPacket{}, c.opts.ProtoVersion)
	if err != nil {
		return err
	}
...
func (c *Client) handlePacket(frame okproto.Frame) error {

	c.InBytes.Add(uint64(frame.GetFrameSize()))
	c.InMsgs.Inc()

	switch frame.GetFrameType() {
	case okproto.SENDACK: // 发送回执
		c.handleSendackPacket(frame.(*okproto.SendackPacket))
	case okproto.RECV: // 收到消息
		c.handleRecvPacket(frame.(*okproto.RecvPacket))
	case okproto.PONG: // pong
		c.handlePong()
	}
	return nil
}

func (c *Client) handleSendackPacket(packet *okproto.SendackPacket) {
	if c.onSendack != nil {
		c.onSendack(packet)
	}
}
...
// 处理接受包
func (c *Client) handleRecvPacket(packet *okproto.RecvPacket) {
	var err error
	var payload []byte
	if c.onRecv != nil {
		if !packet.Setting.IsSet(okproto.SettingNoEncrypt) {
			payload, err = okutil.AesDecryptPkcs7Base64(packet.Payload, []byte(c.aesKey), []byte(c.salt))
			if err != nil {
				panic(err)
			}
	...
	if err == nil {
		c.sendPacket(&okproto.RecvackPacket{
...
func (c *Client) SendMessage(channel *Channel, payload []byte, opt ...SendOption) error {
	...
	if !opts.NoEncrypt {
		// 加密消息内容
		newPayload, err = okutil.AesEncryptPkcs7Base64(payload, []byte(c.aesKey), []byte(c.salt))
		if err != nil {
			c.Error("加密消息payload失败！", zap.Error(err))
			return err
		}
	} else {
		setting.Set(okproto.SettingNoEncrypt)
	}

	clientMsgNo := opts.ClientMsgNo
	if clientMsgNo == "" {
		clientMsgNo = okutil.GenUUID() // TODO: uuid生成非常耗性能
	}
	clientSeq := c.clientIDGen.Add(1)
	packet := &okproto.SendPacket{
	...
	if !opts.NoEncrypt {
		...
		actMsgKey, err := okutil.AesEncryptPkcs7Base64([]byte(signStr), []byte(c.aesKey), []byte(c.salt))
		if err != nil {
			c.Error("加密数据失败！", zap.Error(err))
			return err
		}
		packet.MsgKey = okutil.MD5(string(actMsgKey))
	}
...
func (c *Client) sendConnect() error {
	...
	c.clientPrivKey, clientPubKey = okutil.GetCurve25519KeypPair() // 生成服务器的DH密钥对
	packet := &okproto.ConnectPacket{
		Version:         c.opts.ProtoVersion,
		DeviceID:        okutil.GenUUID(),
		DeviceFlag:      okproto.APP,
	...
	connack, ok := f.(*okproto.ConnackPacket)
	if !ok {
		return errors.New("返回包类型有误！不是连接回执包！")
	}
	if connack.ReasonCode != okproto.ReasonSuccess {
	...
	shareKey := okutil.GetCurve25519Key(c.clientPrivKey, serverPubKey) // 共享key
	c.aesKey = okutil.MD5(base64.StdEncoding.EncodeToString(shareKey[:]))[:16]

	return nil
}

// 发送包
func (c *Client) sendPacket(packet okproto.Frame) error {
...
func (c *Client) appendPacket(packet okproto.Frame) error {
