...
import (
	...
	"github.com/samlau0508/imserver/pkg/oknet/crypto/tls"
	okproto "github.com/samlau0508/imserver/pkg/proto"
	"github.com/samlau0508/imserver/pkg/ring"
...
func GetDefaultConn(id int64, connFd NetFd, localAddr, remoteAddr net.Addr, eg *Engine, reactorSub *ReactorSub) *DefaultConn {
	...
	defaultConn.Log = oklog.NewOKLog(fmt.Sprintf("Conn[[reactor-%d]%d]", reactorSub.idx, id))
...
func CreateConn(id int64, connFd NetFd, localAddr, remoteAddr net.Addr, eg *Engine, reactorSub *ReactorSub) (Conn, error) {
	...
	// 	valueMap:   map[string]interface{}{},
	// 	uptime:     time.Now(),
	// 	Log:        oklog.NewOKLog(fmt.Sprintf("Conn[%d]", id)),
	// }
...
func (d *DefaultConn) String() string {
	...
	return fmt.Sprintf("Conn[%d] uid=%s fd=%d deviceFlag=%s deviceLevel=%s deviceID=%s", d.id, d.uid, d.fd, okproto.DeviceFlag(d.deviceFlag), okproto.DeviceLevel(d.deviceLevel), d.deviceID)
}
