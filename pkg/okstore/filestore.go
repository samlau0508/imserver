...
func (f *FileStore) Open() error {
	...
	var err error
	f.db, err = bolt.Open(filepath.Join(f.cfg.DataDir, "im.db"), 0755, &bolt.Options{Timeout: 10 * time.Second})
	if err != nil {
		return err
	}
...
func (f *FileStore) GetChannel(channelID string, channelType uint8) (*ChannelInfo, error) {
	...
	err = okutil.ReadJSONByByte(value, &data)
	if err != nil {
		return nil, err
	}
...
func (f *FileStore) GetUserToken(uid string, deviceFlag uint8) (string, uint8, error) {
	...
	err = okutil.ReadJSONByByte(value, &resultMap)
	if err != nil {
		return "", 0, err
	}
...
func (f *FileStore) UpdateUserToken(uid string, deviceFlag uint8, deviceLevel uint8, token string) error {
	slotNum := f.slotNum(uid)
	return f.set(slotNum, []byte(f.getUserTokenKey(uid, deviceFlag)), []byte(okutil.ToJSON(map[string]string{
...
func (f *FileStore) SyncMessageOfUser(uid string, startMessageSeq uint32, limit int) ([]Message, error) {
	...
	return f.FileStoreForMsg.LoadNextRangeMsgs(fmt.Sprintf("%s%s", UserQueuePrefix, uid), okproto.ChannelTypePerson, startMessageSeq, 0, limit)
}
...
func (f *FileStore) AddOrUpdateChannel(channelInfo *ChannelInfo) error {
	...
	return f.set(slotNum, []byte(f.getChannelKey(channelInfo.ChannelID, channelInfo.ChannelType)), []byte(okutil.ToJSON(channelInfo.ToMap())))
}
...
func (f *FileStore) UpdateMessageOfUserCursorIfNeed(uid string, messageSeq uint32) error {
	slot := f.slotNum(uid)
	lastSeq := f.getTopic(fmt.Sprintf("%s%s", UserQueuePrefix, uid), okproto.ChannelTypePerson).getLastMsgSeq()
...
func (f *FileStore) AddOrUpdateConversations(uid string, conversations []*Conversation) error {
	...
	return f.db.Update(func(t *bolt.Tx) error {
		...
		return bucket.Put([]byte(key), []byte(okutil.ToJSON(newConversations)))
	})
}
...
func (f *FileStore) GetConversations(uid string) ([]*Conversation, error) {
	...
	err := f.db.View(func(t *bolt.Tx) error {
		...
		if len(value) > 0 {
			err = okutil.ReadJSONByByte(value, &conversations)
			return err
		}
		return nil
...
func (f *FileStore) getSlotBucketWithKey(key string, t *bolt.Tx) (*bolt.Bucket, error) {
	slot := okutil.GetSlotNum(f.cfg.SlotNum, key)
...
func (f *FileStore) slotNum(key string) uint32 {
	return okutil.GetSlotNum(f.cfg.SlotNum, key)
}
...
func (f *FileStore) slotNumForChannel(channelID string, channelType uint8) uint32 {
	return okutil.GetSlotNum(f.cfg.SlotNum, channelID)
}
