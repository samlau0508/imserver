...
func NewFileStoreForMsg(cfg *StoreConfig) *FileStoreForMsg {
	...
	f := &FileStoreForMsg{
		...
		Log:     oklog.NewOKLog("FileStoreForMsg"),
	}
	return f
}
...
func (f *FileStoreForMsg) AppendMessagesOfUser(uid string, msgs []Message) (seqs []uint32, err error) {
	seqs, _, err = f.getTopic(fmt.Sprintf("%s%s", UserQueuePrefix, uid), okproto.ChannelTypePerson).appendMessages(msgs)
	return
}
...
func (f *FileStoreForMsg) getTopic(channelID string, channelType uint8) *topic {
	...
	slotNum := okutil.GetSlotNum(f.cfg.SlotNum, topic)
	f.slotMapLock.RLock()
