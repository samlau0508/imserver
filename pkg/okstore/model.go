...
func NewConversationSet(data []byte) ConversationSet {
	...
	decoder := okproto.NewDecoder(data)
...
func decodeConversation(decoder *okproto.Decoder) (*Conversation, error) {
	// proto version
	_, err := decoder.Uint8()
	if err != nil {
		return nil, err
...
func (s *StreamMeta) Encode() []byte {
	return []byte(okutil.ToJSON(s))
}

func (s *StreamMeta) Decode(data []byte) error {

	return okutil.ReadJSONByByte(data, s)
}
...
func currentStreamItem(reader io.ReaderAt, startOffset int64, readBlob bool) (*StreamItem, int, error) {
	...
	// client msg no
	clientMsgNoLenBytes := make([]byte, okproto.StringFixLenByteSize)
